...
class NetworkManager extends EventEmitter {
  ...
  async setAsyncCallStackDepth(maxDepth) {
    await this._client.send('Debugger.enable');
    await this._client.send('Debugger.setAsyncCallStackDepth', { maxDepth });
  }
   ...
   */
  _handleRequestStart(requestId, interceptionId, url, resourceType, requestPayload, frameId, redirectChain, initiator) {
    let frame = null;
    if (frameId)
      frame = this._frameManager.frame(frameId);
    const request = new Request(this._client, requestId, interceptionId, this._userRequestInterceptionEnabled, url, resourceType, requestPayload, frame, redirectChain, initiator);
    if (requestId)
  ...
  _onRequestWillBeSent(event) {
    ...
    this._handleRequestStart(event.requestId, null, event.request.url, event.type, event.request, event.frameId, redirectChain, event.initiator);
  }
...
class Request {
   ...
   * @param {!Array<!Request>} redirectChain
   */
  constructor(client, requestId, interceptionId, allowInterception, url, resourceType, payload, frame, redirectChain, initiator) {
    this._client = client;
